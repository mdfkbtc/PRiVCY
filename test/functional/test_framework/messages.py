...
def privcyhash(s):
    return privcy_hash.getPoWHash(s)
...
# Objects that map to privcyd objects, which can be serialized/deserialized
...
class CBlockHeader():
    ...
    def calc_sha256(self):
        if self.sha256 is None:
            ...
            r += struct.pack("<I", self.nBits)
            r += struct.pack("<I", self.nNonce)
            self.sha256 = uint256_from_str(privcyhash(r))
            self.hash = encode(privcyhash(r)[::-1], 'hex_codec').decode('ascii')
...
class msg_headers():
    ...
    def deserialize(self, f):
        # comment in privcyd indicates these should be deserialized as blocks
