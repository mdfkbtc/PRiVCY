...
# privcy_hash
RUN git clone https://github.com/privcypay/privcy_hash
RUN cd privcy_hash && python3 setup.py install
...
RUN groupadd -g ${GROUP_ID} privcy
RUN useradd -u ${USER_ID} -g privcy -s /bin/bash -m -d /privcy privcy
...
RUN mkdir /privcy-src && \
  mkdir -p /cache/ccache && \
  mkdir /cache/depends && \
  mkdir /cache/sdk-sources && \
  chown $USER_ID:$GROUP_ID /privcy-src && \
