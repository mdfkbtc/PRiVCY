...
const char * const BITCOIN_CONF_FILENAME = "privcy.conf";
const char * const BITCOIN_PID_FILENAME = "privcyd.pid";
...
public:
    ...
    static inline std::pair<bool,std::string> GetArg(const ArgsManager &am, const std::string& arg)
    {
        ...
        // argument value seen from the command line (so "privcyd -foo=bar
...
{
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\PRiVCYCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\PRiVCYCore
    // Mac: ~/Library/Application Support/PRiVCYCore
    // Unix: ~/.privcycore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "PRiVCYCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/PRiVCYCore";
#else
    // Unix
    return pathRet / ".privcycore";
#endif
#endif
}
...
void ArgsManager::ReadConfigFile(const std::string& confPath)
{
    ...
    if (stream.good()) {
        ...
        // Create an empty privcy.conf if it does not excist
...
std::string CopyrightHolders(const std::string& strPrefix, unsigned int nStartYear, unsigned int nEndYear)
{
    ...
    // Check for untranslated substitution to make sure PRiVCY Core copyright is not removed by accident
    if (strprintf(COPYRIGHT_HOLDERS, COPYRIGHT_HOLDERS_SUBSTITUTION).find("PRiVCY Core") == std::string::npos) {
        strCopyrightHolders += "\n" + strPrefix + strprintf(" %u-%u ", 2014, nEndYear) + "The PRiVCY Core developers";
    }
